...
int match_(struct query_expression* qe, char* text)
{
    ...
    } else if (qe->query_word_num == 4) {
        if (qe->logic_ops[0] == '#' && qe->logic_ops[1] == '#' && qe->logic_ops[2] == '|') {         // | | |
            return (qe->tf[0] || qe->tf[1] || qe->tf[2] || qe->tf[3]); 
        } else if (qe->logic_ops[0] == '|' && qe->logic_ops[1] == '#' && qe->logic_ops[2] == '&') {  // | | &
            return (qe->tf[0] || qe->tf[1] || qe->tf[2] && qe->tf[3]);
        } else if (qe->logic_ops[0] == '#' && qe->logic_ops[1] == '&' && qe->logic_ops[2] == '#') {  // | & |
            return (qe->tf[0] || qe->tf[1] && qe->tf[2] || qe->tf[3]);
        } else if (qe->logic_ops[0] == '#' && qe->logic_ops[1] == '&' && qe->logic_ops[2] == '&') {  // | & &
...
static void print_readable_logic_query_result(struct logic_query_result* result)
{
    int i;

    printf("hit count: %d\n", result->hit_count);

    for (i = 0; i < result->hit_count; i++) {
	print_query_expression(query_rule_table->table[result->hit_rule_list[i]]);	
    }
}
