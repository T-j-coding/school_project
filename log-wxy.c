...
int sendlog(struct loginfo * log, int monitor)
{
    //printlog(log);
    char buf[1200];
...
//    sprintf(buf, "%d|",log->configid);
    ...
    sprintf(buf+strlen(buf), "%s|", log->serverip);
    sprintf(buf+strlen(buf), "%u|", log->serverport);
    sprintf(buf+strlen(buf), "%s|", log->clientip);
    ...
    // sunpy: remove temperorily for blocking at this point
    char recvbuf[100];
...
void initlog(struct loginfo * log, struct location_tuple * lt)
{
...
//    log->clientip  =  a_tcp->addr.saddr;
    ...
    log->serverport = lt->serverport;

    struct in_addr server_ip_addr;
    server_ip_addr.s_addr = lt->serverip;
    memset(log->serverip, 0, sizeof(log->serverip));
    strcpy(log->serverip, inet_ntoa(server_ip_addr));

    log->clientport = lt->clientport;

    struct in_addr client_ip_addr;
    client_ip_addr.s_addr = lt->clientip;
    memset(log->clientip, 0, sizeof(log->clientip));
    strcpy(log->clientip, inet_ntoa(client_ip_addr));
