...
int addstr_N(const char* s)
{
        int length = strlen(s);
        char* string = (char*)malloc(length+1);
        bzero(string, length+1);
        memcpy(string, s, length);
            acsmAddPattern(NB, (unsigned char*)string, length);
            NA->acsmPatterns=NB->acsmPatterns;
            acsmCompile(NB);
            ACSM_STRUCT * C=NA;
            NA=NB;
            NB=C;
...
int delstr_N(const char* s)
{
        ...
        acsmDelPattern(NB, string, length);
        free(string);
...
int delurl(const char* s)
{
        ...
        acsmDelPattern(URL_B, string, length);
        free(string);
...
int init_ac()
{
	...
	acsmCompile(R);*/
        int err;char errbuf[2048];
        char* pattern="POST /|GET /|OPTIONS /|HEAD /|PUT /|CONNECT /|DELETE /|TRACE /";
        if(regcomp(&regForHttp,pattern,REG_EXTENDED)<0){
            regerror(err,&regForHttp,errbuf,sizeof(errbuf));
            printf("err:%s\n",errbuf);
        }else{printf("REG compile success!\n");}
}
...
int kw_match_N(const char* s)
{
        int length = strlen(s);
        char* string = (char*)malloc(length+1);
        bzero(string, length+1);
        memcpy(string, s, length);
        int found = acsmSearch(NA, (unsigned char*)string, length);
        free(string);
//printf("found:%d\n",found);
        return found;
        }
...
int get_keywords_N(const char* s, char* keywords[])
{
	...
	bzero(string,length+1);
	memcpy(string,s,length);
	int found = acsmSearch_kw(NA,(unsigned char*)string,length,keywords);
	free(string);
	return found;
}
/*
